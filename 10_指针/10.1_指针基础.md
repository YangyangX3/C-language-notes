...
[^2]: 在 C 语言中，数组名本质上是指向整块内存区域的“符号常量”：
	- **自动退化（decay）**  
	  数组名在大多数上下文中会退化为指向首元素的指针，但它本身并不是一个可修改的变量，而是一个内存地址的标识符。
	- **不可赋值**
	  它不能出现在赋值运算符的左侧，因为它不是一个可修改的左值(lvalue)，如果允许 `arr1 = arr2;`，就会产生“数组重新分配内存”或“修改起始地址”之类的语义歧义。
